...
def critical_task_reward(
    config, critical_items, allocation_order, batch_ci_pairs, heuristic
):
    allocation_order = (
        allocation_order.numpy().astype(int)
        if torch.is_tensor(allocation_order)
        else allocation_order
    )
    ci_reward_batch_avg = []
    for states, actions, ci_pairs in zip(
        critical_items, allocation_order, batch_ci_pairs
    ):
...
def compute_reward(config, states_batch, len_mask, actions_batch):
    ...
    for states, actions in zip(states_batch, actions_batch):
        avg_occupancy_ratios.append(
            avg_occupancy(bin_size, states, actions, heuristic=config.agent_heuristic)
        )
    return np.array(avg_occupancy_ratios)


def get_benchmark_rewards(
    config, states_generator: StatesGenerator = None, states_batch=None, ci_groups=None
):
    """
    ...
    for state, ci_group in zip(states, ci_groups):
        ...
        nf_ci_reward.append(
            compute_ci_reward(
                config, state, items_order_default, ci_group, heuristic="NF"
            )
        )
        ff_reward.append(
        ...
        ff_ci_reward.append(
            compute_ci_reward(
                config, state, items_order_default, ci_group, heuristic="FF"
            )
        )
        items_order_decreasing = np.flip(np.argsort(state))
        ffd_reward.append(
            avg_occupancy(
                config.bin_size, state, items_order_decreasing, heuristic="FF"
            )
        )
        ffd_ci_reward.append(
            compute_ci_reward(
                config, state, items_order_decreasing, ci_group, heuristic="FF"
            )
        )
